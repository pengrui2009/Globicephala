...
ehm_txbuf_t * ehm_fp_get_txbuf(ehm_envar_t *p_ehm)
{
    ...
    if (!list_empty(p_txbuf_free_list)) {
        ...
        list_del(&txbuf->list);
        txbuf->data_ptr = txbuf->buffer + EHM_BUF_HEAD_RESERVED_LEN;
        txbuf->data_len = 0;
...
{
    return ehm_fp_get_txbuf(&ehm_envar);
}
/*****************************************************************************
 @funcname: ehm_main_proc
 @brief   : ehm module main handle process
...
ehm_rxbuf_t * ehm_fp_get_rxbuf(ehm_envar_t *p_ehm)
{
    ...
    if (!list_empty(p_rxbuf_free_list)) {
        ...
        list_del(&rxbuf->list);
        rxbuf->data_ptr = rxbuf->buffer + EHM_BUF_HEAD_RESERVED_LEN;
        rxbuf->data_len = 0;
...
/*****************************************************************************
 ...
 @return  :
*****************************************************************************/
ehm_rxbuf_t *ehm_get_rxbuf(void)
{
    return ehm_fp_get_rxbuf(&ehm_envar);
}
