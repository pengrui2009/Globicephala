...
#define OSAL_MODULE_DEBUG_LEVEL OSAL_DEBUG_ERROR
...
static int vbd_judge(vsa_node_st *p_node)
{
    ...
    {
    	OSAL_DBGPRT(OSAL_DEBUG_INFO, "p_node->vam_alert:%04x\n", p_node->vam_alert);
        return VSA_ID_NONE;
    }
    
    if (p_node->local_speed <= p_vsa->working_param.danger_detect_speed_threshold)
    {
    	OSAL_DBGPRT(OSAL_DEBUG_INFO, "p_node->local_speed:%.lf p_vsa->working_param.danger_detect_speed_threshold=%.lf\n", p_node->local_speed, p_node->local_speed);
        return VSA_ID_NONE;
    }
    ...
    {
    	OSAL_DBGPRT(OSAL_DEBUG_INFO, "p_node->h_offset: p_vsa->working_param.lane_dis:%d\n", p_node->h_offset, p_vsa->working_param.lane_dis);
        return VSA_ID_NONE;    
    }
    ...
    if((10 < p_node->relative_dir) && (p_node->relative_dir < 350))
    {
       OSAL_DBGPRT(OSAL_DEBUG_INFO, "p_node->relative_dir:%.lf p_node->relative_dir:%.lf%\n", p_node->relative_dir, p_node->relative_dir);
       return VSA_ID_NONE;
    }
    ...
    if (p_node->linear_distance <= 0)
    {
    	OSAL_DBGPRT(OSAL_DEBUG_INFO, "p_node->linear_distance:%d\n", p_node->linear_distance);
        return VSA_ID_NONE;
    }
    
    OSAL_DBGPRT(OSAL_DEBUG_INFO, "alert value:%d\n", VSA_ID_VBD);
    return VSA_ID_VBD;
...
static int cfcw_judge(vsa_node_st *p_node)
{
    ...
    if(p_node->local_speed <= p_vsa->working_param.danger_detect_speed_threshold)
    {
    	OSAL_DBGPRT(OSAL_DEBUG_INFO, "p_node->local_speed:%f p_vsa->working_param.danger_detect_speed_threshold=%d\n", p_node->local_speed, p_vsa->working_param.danger_detect_speed_threshold);
        return VSA_ID_NONE;
    }
    ...
    if((10 < p_node->relative_dir) && (p_node->relative_dir < 350))
    {
    	OSAL_DBGPRT(OSAL_DEBUG_INFO, "p_node->relative_dir:%f\n", p_node->relative_dir);
        return VSA_ID_NONE;
    }
    ...
    {
    	OSAL_DBGPRT(OSAL_DEBUG_INFO, "p_node->safe_distance:%d p_node->linear_distance:%d\n", p_node->safe_distance, p_node->linear_distance);
        return VSA_ID_NONE;
    }
