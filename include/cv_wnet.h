...
typedef struct _wnet_txinfo {
    ...
    uint32_t timestamp; /* time of generated message,uint minutes of one year */
...
typedef struct _wnet_rxinfo {
    ...
    uint8_t encryption; /* 0 - none, 1 - encpypted */
    uint8_t rssi;  
    uint32_t timestamp; /* time of received message, uint minutes of one year */

    void *extension; /* reserved for future */
} wnet_rxinfo_t;

#define TXBUF_LENGTH     256
#define RXBUF_LENGTH     256               
