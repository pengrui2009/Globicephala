...
{
    int result = 0;
    MessageFrame_t         *msg_frame_ptr = NULL;
    ...
    msg_frame_ptr = calloc(1, sizeof(*msg_frame_ptr));
    if(NULL == msg_frame_ptr)
    {
        result = -ERR_NOMEM;
        goto ERR_EXIT;
    }
    ...
    {
        msg_frame_ptr->present = MessageFrame_PR_bsmFrame;
        result = MSG_BasicSafetyMessage_allocate(&(msg_frame_ptr->choice.bsmFrame), &msg_ptr->msg.msg_bsm);
        break;
    }
    case MSG_MessageFrame_ID_MAPDATA:
    {
        msg_frame_ptr->present = MessageFrame_PR_mapFrame;
        result = MSG_MapData_allocate(&(msg_frame_ptr->choice.mapFrame), &msg_ptr->msg.msg_mapdata);
        break;
    }
    case MSG_MessageFrame_ID_RSI:
    {
        msg_frame_ptr->present = MessageFrame_PR_rsiFrame;
        result = MSG_RoadSideInformation_allocate(&(msg_frame_ptr->choice.rsiFrame), &msg_ptr->msg.msg_rsi);
        break;
    }
    case MSG_MessageFrame_ID_RSM:
    {
        msg_frame_ptr->present = MessageFrame_PR_rsmFrame;
        result = MSG_RoadSideSafetyMessage_allocate(&(msg_frame_ptr->choice.rsmFrame), &msg_ptr->msg.msg_rsm);
        break;
    }
    case MSG_MessageFrame_ID_SPAT:
    {
        msg_frame_ptr->present = MessageFrame_PR_spatFrame;
        result = MSG_SignalPhaseAndTiming_allocate(&(msg_frame_ptr->choice.spatFrame), &msg_ptr->msg.msg_spat);
        break;
    }
    }
    
    /* Encode message to UPER format. */
    encode_rval = uper_encode_to_buffer(&asn_DEF_MessageFrame, 0, msg_frame_ptr, buffer_ptr, (size_t)buffer_size);
