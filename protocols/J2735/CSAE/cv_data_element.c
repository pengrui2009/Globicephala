...
const static bsctyp_transf_st BasicTypeGroup[BSCTYP_DEF_EN_MAX] = 
{
...
/* Allocate room for outer type data and encode basic type data. */
int bsctyp_allocate_encode(bsctyp_def_en type, outertyp_t_ptr *outer_data_ptr_ptr, innertyp_t inner_data)
{
    outertyp_t *outer_data_ptr = NULL;
    

    /* Allocate room for outer data. */
    outer_data_ptr = CALLOC(1, sizeof(*outer_data_ptr));
    if(outer_data_ptr != NULL)
    {
        * outer_data_ptr = bsctyp_encode(type, inner_data);
    }
    else
    {
        goto ERR_EXIT;
    ...
    *outer_data_ptr_ptr = outer_data_ptr;
    
    return 0;

ERR_EXIT:

    *outer_data_ptr_ptr = NULL;
    return -1;
}


/* Parse routine for basic type data. */
int bsctyp_parse(bsctyp_def_en type, outertyp_t_ptr outer_data_ptr, innertyp_t_ptr inner_data_ptr)
{
    /* DYear. */
    if(time_ptr->year != NULL)
    {
        DDate_ptr->opt.DYear = MSG_OPTIONAL_YES;
        DDate_ptr->year = bsctyp_decode(DE_DYear, *time_ptr->year);
    }
    else
    {
        DDate_ptr->opt.DYear = MSG_OPTIONAL_NO;
    }
