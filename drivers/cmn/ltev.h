...
/* Send data to ltev module. return error code when error occur or data length been sended. */
extern int ltev_send(drv_ltev_st_ptr ltev_ptr, ltev_infor_st_ptr infor_ptr, uint8_t *buffer_ptr, uint16_t data_len);

/* Receive data from ltev module. Caution: User should give enough data length value to store data. */
extern int ltev_receive(drv_ltev_st_ptr ltev_ptr, ltev_infor_st_ptr infor_ptr, uint8_t *buffer_ptr, uint16_t *data_len_ptr);

/* Init ltev module. */
extern int ltev_init(drv_ltev_st_ptr ltev_ptr, ltev_config_st_ptr config_ptr);

/* Deinit ltev module. */
extern int ltev_deinit(drv_ltev_st_ptr ltev_ptr);
