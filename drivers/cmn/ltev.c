...
*
*END*********************************************************************/
int drv_ltev_device_receive_data(drv_ltev_st_ptr ltev_ptr, ltev_infor_st_ptr infor_ptr, uint8_t *buffer_ptr, uint16_t *data_len_ptr)
{
	int              ret = 0x00;
    ...
    || (buffer_ptr == NULL) || (data_len_ptr == NULL))
    {
        printf("[%s %d]: Invalid parameter. \n", __FUNCTION__, __LINE__);
    ...
    {   
        ...
        printf("[%s %d]: Data payload length over then user buffer. Payload = %d, Userbuff = %d. \n", \
                                                    __FUNCTION__, __LINE__, payload_len, (*data_len_ptr));
        ret = ERR_NODISK;
        goto ERR_EXIT;
    }
...
*
*END*********************************************************************/
int drv_ltev_socket_receive_data(drv_ltev_st_ptr ltev_ptr, ltev_infor_st_ptr infor_ptr, uint8_t *buffer_ptr, uint16_t *data_len_ptr)
{
	int              ret = 0x00;
	ltev_dheader_st head = { { 0 }, { 0 }, 0 };
	uint16_t payload_len = 0x00;
...
* 
* Returned : Negative error code when failure or data length been sended. 
* Comments : Send data to ltev module. 
*
*END*********************************************************************/
int ltev_send(drv_ltev_st_ptr ltev_ptr, ltev_infor_st_ptr infor_ptr, uint8_t *buffer_ptr, uint16_t data_len)
{
    int          ret = 0x00;
...
*
*END*********************************************************************/
int ltev_receive(drv_ltev_st_ptr ltev_ptr, ltev_infor_st_ptr infor_ptr, uint8_t *buffer_ptr, uint16_t *data_len_ptr)
{
    int          ret = 0x00;
    ...
    if(ltev_ptr->config.dev_mode == LTEV_MODE_DEVICE)
    {
        ret = drv_ltev_device_receive_data(ltev_ptr, infor_ptr, buffer_ptr, data_len_ptr);
    }
    else if(ltev_ptr->config.dev_mode == LTEV_MODE_SOCKET)
    {
        ret = drv_ltev_socket_receive_data(ltev_ptr, infor_ptr, buffer_ptr, data_len_ptr);
    }
    else
    {
        printf("[%s %d]: Do not support letv mode. Ltev mode = %d. \n", __FUNCTION__, __LINE__, ltev_ptr->config.dev_mode);
...
/* Init ltev module. */
int ltev_init(drv_ltev_st_ptr ltev_ptr, ltev_config_st_ptr config_ptr)
{
	int     ret = 0x00;


    /* Error detection. */
    if((ltev_ptr == NULL) || (config_ptr == NULL))
    {
        printf("[%s %d]: Invalid parameter. \n", __FUNCTION__, __LINE__);
        ret = -ERR_INVAL;
        goto ERR_EXIT;
    ...
    /* Set configuration. */
    memcpy(&(ltev_ptr->config), config_ptr, sizeof(ltev_ptr->config));
...
ERR_EXIT:
    ...
    ltev_deinit(ltev_ptr);
    return ret;
}


/* Deinit ltev module. */
int ltev_deinit(drv_ltev_st_ptr ltev_ptr)
{
	int    ret = ERR_OK;
